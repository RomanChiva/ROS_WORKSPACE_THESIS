...
class MPPIConfig(object):
    ...
    calculate_cost_once: bool = False
...
class MPPIPlanner(ABC):
    ...
    def _running_cost(self, state, u, t, obst):
        return self.running_cost(state, u, t, obst)
    ...
    def command(self, state, obst):
        ...
        if self.mppi_mode == 'simple':
            ...
            # Sample noise and compute total cost of each perturbation
            cost_total, states = self._compute_total_cost_batch_simple()
            # Find sample with minimum cost
            beta = torch.min(cost_total)
            # Compute weights of each
            ...
            # Apply the computed optimal perturbation to the control sequence
    ...
    def _compute_rollout_costs(self, perturbed_actions, obst=None):
        """
        ...
        for t in range(T):
            ...
            # check for collision at each stage along the planning horizon
            '''
            if obst is not None:
                obst_x_pos = obst[t][0]
                obst_y_pos = obst[t][1]
                obst_pos = [obst_x_pos, obst_y_pos]
                obst_pos = torch.tensor(obst_pos, device="cuda:0")
                dist = torch.linalg.norm(state[:, 0:2] - obst_pos, axis=1)
                collision_cost[dist < 0.5] += 10
            '''
            ...
            if not self.calculate_cost_once:
                c = self._running_cost(state, u, t, obst)
                collision_cost = torch.zeros_like(c)

                # print("the tensor shape of collision cost is: ", collision_cost.shape)
                ...
                if obst is not None:
                    ...
                    for obstacles in obst.obstacles:
                        for mode in range(len(obstacles.gaussians)):  # iterate over the prediction modes
                            obst_x_pos = obstacles.gaussians[mode].mean.poses[t].pose.position.x
                            obst_y_pos = obstacles.gaussians[mode].mean.poses[t].pose.position.y
                            obst_pos = [obst_x_pos, obst_y_pos]
                            obst_pos = torch.tensor(obst_pos, device="cuda:0")
                            dist = torch.linalg.norm(state[:, 0:2] - obst_pos, axis=1)
                            collision_cost[dist < 1.0] += 10
        ...
        # print("states tensor size is: ", states.shape)

        if self.calculate_cost_once:
            # Get cost for each timestep in every rollout
            c = self._running_cost(states, u, t, obst)
            c = c.reshape(T, K)
        ...
        # Cost Total print maximum and minimum cost
