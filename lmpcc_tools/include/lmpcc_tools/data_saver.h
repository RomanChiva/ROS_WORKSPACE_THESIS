...
public:
    DoubleDataSet(const std::string &&name)
        : DataSet(name)
    {
        data_.reserve(100);
    };
...
public:
    ...
    void AddData(const std::string &&data_name, const T &data_value)
    {
        ...
        if (idx_it == data_lookup_.end())
        {
            if (datasets_.size() > 1e5)
                std::cout << "Warning: Data saver is not saving anymore data, too much different datasets were added already (safety to prevent allocation errors!)" << std::endl;
    ...
    {
        std::string path = ros::package::getPath("lmpcc") + "/matlab_exports/data";
    ...
    void SaveData(const std::string &file_path, const std::string &file_name)
    {
        ...
        if (boost::filesystem::create_directories(file_path))
            ROS_INFO_STREAM("Data Saver: Creating Directory Path: " << file_path);

        std::string folder_path = file_path + "/" + file_name;
        if (add_timestamp_ && boost::filesystem::create_directories(folder_path))
            ROS_INFO_STREAM("Data Saver: Creating Directory Path: " << file_path);
        ...
        if (add_timestamp_)
        {
            ...
            full_file_path = folder_path + "/" + datestamp + "-" + timestamp + ".txt";
        }
        else
        {
            full_file_path = file_path + "/" + file_name + ".txt";
        }
    ...
    // This is hacky
    bool LoadAllData(const std::string &file_path, const std::string &file_name, std::map<std::string, std::vector<double>> &result_scalar, std::map<std::string, std::vector<Eigen::Vector2d>> &result_vector)
    {
        // Setup a file stream
        std::string full_file_path = file_path + "/" + file_name + ".txt";
